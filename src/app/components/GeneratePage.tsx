...
const VALID_TAGS = ['frontend', 'backend', 'fullstack', 'devops', 'mobile', 'data', 'ai'] as const;
...
function parseResponse(raw: string): { data?: ParsedResponse; error?: string } {
  ...
  if (!VALID_TAGS.includes(data.emphasis)) {
    return { error: `Invalid emphasis "${data.emphasis}". Must be one of: ${VALID_TAGS.join(', ')}` };
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">No CV data found. Set up your profile first.</p>
          <a href="/" className="text-indigo-600 underline">← Back</a>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50">
      <header className="border-b border-gray-200 bg-white sticky top-0 z-40 backdrop-blur bg-white/80">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="text-sm text-gray-600 hover:text-gray-900">← Builder</a>
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-500" />
            <h1 className="text-lg font-semibold text-gray-900">Generate tailored CV</h1>
          </div>
          {applied ? (
            <a
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        {applied && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm flex items-start gap-2">
            ...
            <p className="text-gray-900">A generated CV is active in your browser. Apply a new response below to replace it.</p>
          </div>
        )}
        ...
        <StepCard n={1} title="Paste the job description">
          <textarea
            ...
            className="w-full min-h-[180px] rounded-lg border border-gray-300 p-3 text-sm font-mono leading-relaxed focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <p className="text-xs text-gray-500 mt-2">{jd.length.toLocaleString()} characters</p>
        </StepCard>

        {/* Step 2 */}
        <StepCard n={2} title="Copy the prompt and send to Claude">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            >
              Open claude.ai <ExternalLink className="w-3.5 h-3.5" />
            </a>
            <p className="text-xs text-gray-500">Paste in a new chat, hit send, then copy Claude's JSON response.</p>
          </div>
        </StepCard>

        {/* Step 3 */}
        <StepCard n={3} title="Paste Claude's JSON response">
          <textarea
            value={response}
            onChange={(e) => setResponse(e.target.value)}
            placeholder={'{\n  "id": "generated",\n  "label": "...",\n  ...\n}'}
            className="w-full min-h-[160px] rounded-lg border border-gray-300 p-3 text-xs font-mono leading-relaxed focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          {parseError && (
        ...
        {applied && (
          <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-50/40 p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Generated CV ready</h3>
            </div>
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              ...
              <Field label="Skills order" value={applied.skillsOrder.join(' → ')} />
            ...
            <p className="text-xs text-gray-500 mt-3">
              Stored in your browser (localStorage). Visit /?profile=generated and Cmd+P to print.
            </p>
          </div>
        )}
      </main>
...
function Field({ label, value, multiline }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div>
      ...
      <dd className={`text-gray-900 ${multiline ? 'leading-relaxed' : 'truncate'}`}>{value}</dd>
    </div>
  );
}
